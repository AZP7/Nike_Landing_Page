...
function Review() {
  ...
  return (
    <section className='bg-[var(--bg-color)] w-full pb-10'>
        <div className='w-full flex flex-col justify-center mb-6 items-center'>
            ...
            <p className='w-[70%] text-center text-gray-600 mt-6'>Hear genuine from our satisfied customers about their exceptional experiences with us.</p>
        </div>
        <div>
            {customerDetail.map((item,index)=>(
                <div key={index} className='flex flex-col mt-8 justify-center items-center'>
                    <img src={item.icon} alt="user profile" className='w-[20%] mb-5 object-contain rounded-full ' />
                    <p className='w-[70%] text-center'>{item.message}</p>
